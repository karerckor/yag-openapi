...
export function useUpdateTask() {
    ...
    return useMutation({
        mutationFn: ({ id, updates }: { id: number, updates: TaskManagerApi.UpdateTaskDto }) => api.tasks[':id'].$patch({ json: updates, param: { id } }).then(r => r.json()),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['tasks'] });
        }
    });
...
export function useDeleteTask() {
    ...
    return useMutation({
        mutationFn: (id: number) => api.tasks[':id'].$delete({ param: { id } }).then(r => r.json()),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['tasks'] });
        }
    });
